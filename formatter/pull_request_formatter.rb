...
class PullRequestFormatter
  def format(body:)
    ...
    body.each do |pull_request|
      reviewer = build_reviewer(pull_request: pull_request)
      creator = build_creator(pull_request: pull_request)
      pr_to_insert = build_pull_request(creator: creator, reviewer: reviewer, pull_request: pull_request)
  ...
  private def build_reviewer(pull_request:)
    reviewer = User.new
    reviewer.name =
      pull_request['assignee']['login'] unless
        pull_request['assignee'].nil?
    reviewer
  end
