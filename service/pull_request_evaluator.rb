class PullRequestEvaluator
  ...
  def evaluate_pull_requests(pull_requests:)
    pull_requests.each do |pull_request|
      if require_message?(pull_request: pull_request)
        message = format_message(pull_request: pull_request)
        @message_writer.send(message: message, pull_request: pull_request)
      end
    end
  end

  private def require_message?(pull_request:)
    old_pull_request?(pull_request: pull_request) && assignee?(pull_request: pull_request)
  end
  ...
  private def format_message(pull_request:)
    @message_formatter.format_message(pull_request: pull_request)
  end
end
