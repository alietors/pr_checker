...
class PullRequestDownloader
  ...
  def download(owner:, repository:)
    incomplete_pull_requests = download_pull_requests(owner: owner, repository: repository)
    update_pull_requests(incomplete_pull_request: incomplete_pull_requests, owner: owner)
  end

  private def download_pull_requests(owner:, repository:)
    url = build_pull_url(owner: owner, repository: repository)
    response = @rest_wrapper.get(url: url)
    @pull_request_formatter.format(body: response.body)
  end
  ...
  private def update_pull_requests(incomplete_pull_request:, owner:)
    ...
    incomplete_pull_request.each do |pull_request|
      user = get_user_information(user_name: pull_request.creator.name, users_info: users_info)
      pull_request.creator.email = user.email

      user = get_user_information(user_name: pull_request.reviewer.name, users_info: users_info)
      pull_request.reviewer.email = user.email

      pull_request.repo_owner = owner
  ...
  private def get_user_information(user_name:, users_info:)
    unless users_info.include? user_name
      user = download_user_information(user_name: user_name)
  ...
  private def download_user_information(user_name:)
    url = build_user_url(user: user_name)
    response = @rest_wrapper.get(url: url)
    @user_formatter.format(user: response.body)
  end
